...
resource "okta_policy_rule_signon" "SignOnRuleNetwork" {
  ...
  network_excludes = [okta_network_zone.AIG.id, okta_network_zone.AIG_Corporate.id, okta_network_zone.AIG_Japan.id, okta_network_zone.AIG_Korea-Business.id, okta_network_zone.AIG_Korea_VDI.id, okta_network_zone.Alight.id, okta_network_zone.BIZPRO.id, okta_network_zone.NIE-Validus.id]
  access = "DENY"
  priority = "1"
}
